...
$combine = array(
    ...
    "/wa-notify-banner-simple-purple.png" => array("iOS installs")
);
$show = array("iOS installs",
// "/",
// "/uw",
// "/FAQ",
// "/privacy-policy",
...
$launch_date = (new DateTime('October 1, 2021'));
$offset_months = $argv[1] ?? 0;
$offset_di = new DateInterval('P'.$offset_months.'M');
$start_date = $launch_date->add($offset_di);

// Set to last day of start date's month
$end_date = new DateTime($start_date->format('F')." ".cal_days_in_month(CAL_GREGORIAN,$start_date->format('n'),$start_date->format('Y')).", ".$start_date->format('Y'));
// $start_date = date_create_from_format('Y-m-d H:i:s', '2020-11-01 00:00:00');

$site_out = "./site_stats_$site_interval.test.{$start_date->format('Y.m.d')}.json";
$resource_out = "./resource_stats_test_$resource_interval.test.{$start_date->format('Y.m.d')}.json";

$program_start_dt = new DateTime();
// echo "Start time: {$program_start_dt->format('Y-m-d H:i:s')}\n";
...
// }
$resource_data = array(
    'start_time' => $min_date->format('Y-m-d G:i:s'),
    'end_time' => $max_date->format('Y-m-d G:i:s'),
    'data' => $resource_stats
);
